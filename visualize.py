...
present_subareas = [k for k in ["A", "B", "C", "D", "E", "F", "G"]
                    if k in safety_df["subarea"].values]
...
colors_detail = plt.cm.Set3(np.linspace(0, 1, max(len(subdomain_counts), 1)))
...
def plot_major(ax):
    bars = ax.bar(major_names, major_counts.values, color=colors_major,
...
def plot_subareas(ax):
    bars = ax.barh(subarea_names[::-1], subarea_counts.values[::-1],
                   color=colors_subarea[:len(subarea_counts)][::-1],
                   edgecolor="white", linewidth=0.8)
...
def plot_scores(ax):
    s4 = df[df["class"] == 4]
    score_counts = s4["total_score"].value_counts().sort_index()
...
plot_scores(ax4)
fig4.suptitle("ICLR 2026 — Safety Score Distribution", fontsize=15, fontweight="bold", y=0.98)
fig4.tight_layout(rect=[0, 0, 1, 0.95])
fig4.savefig("plot_score_distribution.png", dpi=150, bbox_inches="tight", facecolor="white")
plt.close(fig4)
