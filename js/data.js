...
/* Research orgs behind safety papers — data/org_plots/orgs.csv (full-text scan + LLM-verified
   affiliations). Value = association count (any confirmed affiliation; one paper can add to
   several orgs); [name, associations, legal type]. Excludes University/Independent (untracked). */
const ORGS_TOP = [
  ['Google DeepMind', 106, 'corporate'],
  ['OpenAI', 71, 'corporate → PBC (2026)'],
  ['MATS', 60, 'nonprofit fellowship'],
  ['Anthropic', 50, 'public-benefit corporation'],
  ['Mila', 50, 'academic centre'],
  ['Microsoft Research', 41, 'corporate'],
  ['Center for AI Safety', 33, 'nonprofit'],
  ['Shanghai AI Laboratory', 27, 'nonprofit'],
  ['FAR.AI', 23, 'nonprofit'],
  ['Meta AI', 23, 'corporate'],
  ['Vector Institute', 23, 'academic centre'],
  ['UK AI Security Institute', 20, 'government'],
  ...
  ['Stanford Existential Risks Initiative', 3, 'academic centre'],
  ...
  ['METR', 2, 'nonprofit'],
  ['LISA', 2, 'nonprofit'],
  ['AI Safety Institute (generic)', 2, 'government'],
...
/* Papers led by a named research org per year — [year, named-org, checked] from org_verified.csv.
   "checked" = safety papers with retrievable full text scanned for affiliations. */
const ORG_BY_YEAR = [
  ['2019', 3, 9], ['2020', 6, 26], ['2021', 1, 31], ['2022', 4, 48],
  ['2023', 16, 101], ['2024', 51, 370], ['2025', 71, 744], ['2026', 90, 954],
];
...
const VIEWS = {
  ...
  orgs: {
    ...
    kicker: 'ORG AFFILIATIONS', big: { num: 54 }, bigUnit: 'orgs', bigLabel: 'appear as an affiliation on safety papers',
    brief: 'Which organizations are behind the safety papers, from author affiliations, with funders (funding acknowledgments) counted separately. Includes labs, academic groups, nonprofits and government institutes.',
    stats: [{ k: 'Research orgs', v: '54' }, { k: 'Funders', v: '7' }, { k: 'Top org', v: 'DeepMind · 106' }],
    type: 'orgs',
    note: 'Bars count any confirmed affiliation, so a paper authored across two orgs adds to both. 2,283 safety papers were scanned; 348 have no tracked org (university / independent) and are not shown. FUNDERS lists funding acknowledgments separately.',
  },
  papers: {
